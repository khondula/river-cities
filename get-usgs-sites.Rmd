...
cbsa_sf <- sf::st_read(cbsa_file) %>% dplyr::filter(LSAD == "M1")
```

392 Metro areas (50K+)

```{r}
m1_cbsas <- unique(cbsa_sf$GEOID)
my_cbsa <- cbsa_sf[["GEOID"]][1]
```
...
get_usgs_sites <- function(my_cbsa){
  ...
  gauges_dir <- "/nfs/khondula-data/projects/river-cities/data/gauges-cbsa-M1"
...
get_usgs_sites(m1_cbsas[2])

purrr::walk(m1_cbsas[51:0], ~get_usgs_sites(.x))
```
...
```{r, eval = FALSE}
library(rslurm)
pars <- data.frame(my_cbsa = m1_cbsas[51:392],
...
df %>% readr::write_csv("gauges-cbsa-M1.csv")
df %>% readr::write_csv("/nfs/public-data/NFIP/gauges-cbsa-M1.csv")
```
